...
def calculate_max_magic_hit(player, weapon, gear, spell=None):
    """
    Calculates max magic hit for powered staves and spellbook spells.
    - player: player dict
    - weapon: selected weapon dict
    - gear: gearStats dict
    - spell: (optional) spell dict if using a spellbook spell
    """
    magic_level = player["combatStats"]["magic"]
    prayer_magic_bonus = 4
    magic_damage_bonus = (gear["bonuses"].get("magic_str", 0) + prayer_magic_bonus) / 100.0  # e.g., 15 for +15%
    print(magic_damage_bonus)
    base_max_hit = 0
    potion_bonus = 21.0
    

    # Example for Tumeken's Shadow (Charged)
    if weapon and weapon["name"] == "Tumeken's shadow":
        # Shadow multiplier is 3 for charged
        shadow_multiplier = 3
        base = int(magic_level + potion_bonus)
        base_damage = int((base / 3) - 1)
        max_hit = int(base_damage * (1 + min(magic_damage_bonus * shadow_multiplier, 1.0)))
        return max_hit

    # Example for Trident of the Swamp
    if weapon and weapon["category"] == "Powered Staff" and weapon["name"] != "Tumeken's shadow":
        base = int(magic_level + potion_bonus)
        print(f"[Sim] Base Damage: {base}")
        base_damage = int((base / 3) - 1)
        print(f"[Sim] Base Damage: {base_damage}")
        max_hit = int(base_damage * (1 + magic_damage_bonus))
        print(f"[Sim] Max Hit: {max_hit}")
        return max_hit

    # Example for spellbook spell (e.g., Fire Surge)
    # if spell:
    #     base_max_hit = spell["base_max_hit"]
    #     # Apply modifiers as needed (chaos gauntlets, charge, etc.)
    #     # For now, just use base_max_hit
    #     max_hit = int(base_max_hit * (1 + magic_damage_bonus))
    #     return max_hit

    # Fallback: use your old formula
    return 0

def calculate_accuracy_for_style(player, monster, style, gear):
    magic_level = player["combatStats"]["magic"]
    potion_bonus = 21.0
    prayer_magic_bonus = 1.25
    style_bonus = style.get("magic", 0)
    void_bonus = 1.0
    effective_magic = int(((magic_level + potion_bonus) * prayer_magic_bonus + style_bonus + 8.0) * void_bonus)
    attack_type = style.get("attack_type", "").lower()
    print(f"[Sim] Attack Type: {attack_type}")
    equipment_bonus = {
    ...
    print(f"[Sim] Effective Magic Level: {effective_magic}")
    print(f"[Sim] Attack Bonus: {attack_bonus}")
    print(f"[Sim] Style Bonus: {style_bonus}")
    max_attack_roll = effective_magic * (attack_bonus + 64)
    print(f"[Sim] Max Attack Roll: {max_attack_roll}")
    defence_bonus = {
    ...
    print(f"[Sim] Max Defence Roll: {max_defence_roll}")
    ...
    return accuracy, effective_magic, max_attack_roll, max_defence_roll

# def calculate_magic_accuracy(player, monster, style, gear):
#     # Player's effective magic level (with prayer)
#     magic_level = player["combatStats"]["magic"]
#     prayer_magic_bonus = 1.0  # Set to 1.0 unless you want to simulate prayer
#     effective_magic = int(magic_level * prayer_magic_bonus + 8)
#     magic_attack_bonus = gear["offensive"]["magic"]

#     # Player's attack roll
#     attack_roll = effective_magic * (magic_attack_bonus + 64)

#     # Monster's effective defence level
#     monster_magic_level = monster["skills"]["magic"]
#     monster_defence_level = monster["skills"]["def"]
#     effective_defence = int(monster_magic_level * 0.7 + monster_defence_level * 0.3 + 8)
#     monster_magic_defence_bonus = monster["defensive"]["magic"]

#     # Monster's defence roll
#     defence_roll = effective_defence * (monster_magic_defence_bonus + 64)

#     # Hit chance formula
#     if attack_roll > defence_roll:
#         accuracy = 1.0 - (defence_roll + 2) / (2.0 * (attack_roll + 1))
#     else:
#         accuracy = attack_roll / (2.0 * (defence_roll + 1))

#     return accuracy, effective_magic, attack_roll, defence_roll

def find_best_magic_style(player, monster):
    mage = player["gearSets"]["mage"]
    weapon = mage.get("selectedWeapon")
    if weapon:
        for style in weapon["weapon_styles"]:
            max_hit = calculate_max_magic_hit(player, weapon, mage["gearStats"], style.get("spell"))
            accuracy, effective_magic, attack_roll, defence_roll = calculate_accuracy_for_style(player, monster, style, mage["gearStats"])
            ...
            style_result = {
                ...
                # "effective_magic": effective_magic,
                "effective_magic": effective_magic,
                "attack_roll": attack_roll,
                "defence_roll": defence_roll,
            }
            return style_result  # Just return the first style for now
    return None
...
if __name__ == "__main__":
    ...
    with open("C:\\Users\\Dylan\\osrs_sim\\test_vesp_payload.json", "r") as f:
        payload = json.load(f)
    ...
    best_style = find_best_magic_style(player, monster)
    monster_hp = monster["skills"]["hp"]
    max_hit = best_style["max_hit"]
    print(f"[Sim] Max hit: {max_hit}")
    accuracy = best_style["accuracy"]
    print(f"[Sim] Accuracy: {accuracy:.4f}")
    attack_speed = 4
