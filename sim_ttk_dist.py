...
if __name__ == "__main__":
    ...
    # After computing kill_prob (cumulative)
    per_attack_prob = np.diff(np.insert(kill_prob, 0, 0))
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=attack_ticks[:capped_idx],
        y=per_attack_prob[:capped_idx],
        mode='lines+markers',
        name='Per-Attack Kill Probability'
    ))
    fig.update_layout(
        title="Per-Attack Kill Probability (Jagged)",
        xaxis_title="Tick",
