...
def calculate_max_hit_for_style(player, monster, style, gear, gear_type="melee"):
    if gear_type == "melee":
        level = player["combatStats"]["strength"]
        potion_bonus = 19.0  # super combat
...
def calculate_accuracy_for_style(player, monster, style, gear, gear_type="melee"):
    if gear_type == "melee":
...
def find_best_combat_style(player, monster, gear_type):
    ...
    # print(f"Best {gear_type} style: {best_style}")
    return best_style