...
def simulate_multi_ttk(monster_stats_list, player, mining_level, attack_speed=5, trials=100_000):
    total_ttks = []
        ...
        for monster in monster_stats_list:
            ...
            while hp > 0:
                ...
                if np.random.rand() < accuracy:
                    ...
                    hit = 0
                hp -= hit
        total_ttks.append(total_ticks)
    return np.mean(total_ttks), np.std(total_ttks)
...
if __name__ == "__main__":
    ...
    for i, monster in enumerate(monsters):
        ...
        expected_damage = accuracy * (sum(j for j in range(0, max_hit + 1)) / (max_hit + 1))
        print(f"[Monster {i+1}] {monster['name']} - Probability of 0 damage: {p_zero:.4f}")
        print(f"[Monster {i+1}] {monster['name']} - Expected damage per attack: {expected_damage:.4f}")

    mean_ttk, std_ttk = simulate_multi_ttk(monsters, player, mining_level, attack_speed=5)
    elapsed = time.time() - start_time
    print(f"Total Expected TTK for {len(monsters)} monsters: {mean_ttk:.2f} ticks ({mean_ttk * 0.6:.2f} seconds)")
