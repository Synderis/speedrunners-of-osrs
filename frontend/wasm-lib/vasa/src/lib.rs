...
pub fn calculate_dps_with_objects_vasa(payload_json: &str) -> String {
    ...
    let initial_delay = if room_methods.len() > 0 && room_methods[0] == "Flame Skip" {
        22
    } else {
        29
    };
    ...
    for i in 0..trials {
        ...
        attack_pattern = if room_methods.len() > 0 && room_methods[0] == "Flame Skip" && voidwaker {
            vec![20, 15, rng.gen_range(33..=36)]
        } else if room_methods.len() > 0 && room_methods[0] == "Flame Skip" {
